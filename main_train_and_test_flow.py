...
parser.add_argument('--dataset', type=str, choices=['PEMS04', 'PEMS08'], default='PEMS04', help='which dataset to run')
parser.add_argument('--trainval_ratio', type=float, default=0.6, help='the ratio of training and validation data among the total')
...
parser.add_argument('--min', type=float, default=-1.23, help='Minimum flow error after standardization, -1.23 for PEMS04 -1.39 for PEMS04')
...
for category in ['train', 'val', 'test']:
    cat_data = np.load(os.path.join('PEMS04', category + '.npz'))
    # cat_data = np.load(os.path.join('PEMS08', category + '.npz'))
